...
import mqtt_prometheus_exporter.handlers as handlers
...
def store(tmp_path, monkeypatch):
    test_store = init_store(str(tmp_path / 'store.json'))
    monkeypatch.setattr(handlers, 'store', test_store)
    return test_store
...
class TestHandlePing:
    def test_basic(self, store):
        ...
        handle_ping(_msg('ping/8.8.8.8', payload))
    ...
    def test_skips_status(self, store):
        handle_ping(_msg('ping/status', b'{}'))
    ...
    def test_missing_key_logs_warning(self, store, caplog):
        import logging
        with caplog.at_level(logging.WARNING):
            handle_ping(_msg('ping/8.8.8.8', json.dumps({'other': 1}).encode()))
        assert len(store['metrics']) == 0
        assert caplog.records
...
class TestHandleRtl433:
    def test_7part_temperature(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/A/42/temperature_C', b'21.5'))
        metrics = dict(store['metrics'])
    ...
    def test_6part_defaults_channel_main(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/111/temperature_C', b'20.0'))
        metrics = dict(store['metrics'])
        assert 'rtl433_temperature_C{channel="main",model="Tower",sensor="outdoor"}' in metrics

    def test_unknown_id_uses_raw(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/A/9999/temperature_C', b'15.0'))
        metrics = dict(store['metrics'])
        assert 'rtl433_temperature_C{channel="A",model="Tower",sensor="9999"}' in metrics

    def test_non_forwarded_field_skipped(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/A/42/temperature_F', b'70.0'))
        assert len(store['metrics']) == 0

    def test_unknown_field_skipped(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/A/42/not_a_field', b'1'))
        assert len(store['metrics']) == 0

    def test_too_few_parts_skipped(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower', b'1'))
        assert len(store['metrics']) == 0

    def test_wrong_third_part_skipped(self, store):
        handle_rtl433(_msg('rtl_433/home/sensors/Tower/A/42/temperature_C', b'20.0'))
        assert len(store['metrics']) == 0

    def test_battery_ok_bool(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/A/42/battery_ok', b'1'))
        metrics = dict(store['metrics'])
    ...
    def test_humidity_forwarded(self, store):
        handle_rtl433(_msg('rtl_433/home/devices/Tower/A/42/humidity', b'65.0'))
...
class TestHandleZigbee2mqtt:
    def test_temperature_produces_c_and_f(self, store):
        ...
        handle_zigbee2mqtt(_msg('zigbee2mqtt/bedroom', payload))
        metrics = dict(store['metrics'])
    ...
    def test_brightness_ttl_3600(self, store):
        ...
        handle_zigbee2mqtt(_msg('zigbee2mqtt/living_room', payload))
    ...
    def test_bad_json(self, store, caplog):
        import logging
        with caplog.at_level(logging.WARNING):
            handle_zigbee2mqtt(_msg('zigbee2mqtt/bedroom', b'not-json'))
        assert len(store['metrics']) == 0
        assert caplog.records
...
class TestHandleWeather:
    ...
    def test_minutely_precipitation_accumulates(self, store):
        ...
        handle_weather(_msg('weather/minutely/0/precipitation', b'1.0'))
        handle_weather(_msg('weather/minutely/1/precipitation', b'2.0'))
        handle_weather(_msg('weather/minutely/2/precipitation', b'0.5'))
    ...
    def test_minutely_resets_on_index_0(self, store):
        import mqtt_prometheus_exporter.handlers as h
        h._minutely_precip_accumulator = 0.0
        handle_weather(_msg('weather/minutely/0/precipitation', b'5.0'))
        handle_weather(_msg('weather/minutely/1/precipitation', b'1.0'))
        # Now index 0 again — should reset
        handle_weather(_msg('weather/minutely/0/precipitation', b'2.0'))
        handle_weather(_msg('weather/minutely/1/precipitation', b'3.0'))
        assert math.isclose(store['metrics']['weather_precipitation_next_hour']['value'], 5.0)

    def test_weather_ttl_is_3600(self, store):
        handle_weather(_msg('weather/daily/0/temp', b'20.0'))
